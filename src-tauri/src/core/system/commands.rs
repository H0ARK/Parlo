...
use crate::core::app::commands::{
    default_data_folder_path, get_parlo_data_folder_path, update_app_configuration,
};
use crate::core::app::constants::{
    PARLO_DATA_DIRS_COMMON, PARLO_DATA_DIRS_CONVERSATIONS, PARLO_DATA_DIRS_MODELS,
    PARLO_DATA_FILES_CONFIGS, PARLO_DATA_FILES_SETTINGS,
};
...
fn delete_conversations(data_folder: &std::path::Path) {
    ...
    for dir in PARLO_DATA_DIRS_CONVERSATIONS {
...
fn delete_models_and_configs(data_folder: &std::path::Path) {
    ...
    for dir in PARLO_DATA_DIRS_MODELS {
        remove_dir(data_folder, dir);
    }
    for file in PARLO_DATA_FILES_CONFIGS {
...
fn delete_common_data(data_folder: &std::path::Path) {
    ...
    for dir in PARLO_DATA_DIRS_COMMON {
        remove_dir(data_folder, dir);
    }
}
...
fn delete_settings(data_folder: &std::path::Path) {
    ...
    for file in PARLO_DATA_FILES_SETTINGS {
        remove_file(data_folder, file);
    }
}
...
fn write_env_to_shell(env_file_path: &str, env_vars: &[(String, String)]) -> Result<(), String> {
    let marker = "# Parlo Local API Server - Claude Code Config";
    ...
    let cleaned: Vec<&str> = existing_content
        .split('\n')
        .filter(|line| {
            // Remove Parlo config markers and existing ANTHROPIC env vars to replace them
            !line.starts_with(marker)
                && !line.starts_with("# Parlo Local API Server")
...
pub fn factory_reset<R: Runtime>(
    ...
    let data_folder = get_parlo_data_folder_path(app_handle.clone());
    log::info!(
...
pub async fn read_logs<R: Runtime>(app: AppHandle<R>) -> Result<String, String> {
    let log_path = get_parlo_data_folder_path(app).join("logs").join("app.log");
    if log_path.exists() {
...
pub fn write_codex_provider_profile_artifacts<R: Runtime>(
    ...
    let env_content = format!(
        "# Generated by Parlo desktop runtime profile manager\nexport CODEX_HOME={}\nexport OPENAI_BASE_URL={}\nexport OPENAI_MODEL={}\nexport OPENAI_API_KEY=\"${{{}:-}}\"\n",
...
pub fn launch_claude_code_with_config(
    ...
    env_vars.push((
        ...
        api_key.unwrap_or_else(|| "Parlo".to_string()),
    ));
    ...
    if cfg!(target_os = "macos") {
            ...
            .open(&env_file_path)
        {
            ...
            Err(_) => {
                ...
                let temp_script_path = format!("{}/.parlo_env_update.sh", home_dir);
            ...
            .open(&env_file_path)
        {
            ...
            Err(_) => {
                let parlo_config_dir = format!("{}/.config/Parlo", home_dir);
                let ext = if shell_name == "bash" { "bash" } else { "zsh" };
                let env_file = format!("{}/claude-code-env.{}", parlo_config_dir, ext);
...
/// Check if the `Parlo` CLI binary is accessible on PATH, or — failing that —
...
#[tauri::command]
pub async fn check_parlo_cli_installed() -> CliInstallStatus {
    ...
    // Fall back to probing the destinations `install_parlo_cli_sync` writes to.
    for candidate in parlo_cli_install_candidates() {
        if candidate.exists() {
            return CliInstallStatus {
                installed: true,
...
/// Paths where `install_parlo_cli_sync` may have placed the `Parlo` binary.
fn parlo_cli_install_candidates() -> Vec<PathBuf> {
    let bin = if cfg!(windows) { "Parlo.exe" } else { "Parlo" };
    ...
    #[cfg(windows)]
    {
        if let Ok(dir) = parlo_cli_bin_dir_windows() {
...
pub fn install_parlo_cli_sync<R: Runtime>(
    ...
    let dest_bin_name = if cfg!(windows) { "Parlo.exe" } else { "Parlo" };
    ...
    if !bundled.exists() && !dest.exists() {
        return Err("Parlo CLI binary not bundled with this version of Parlo.".to_string());
    }
    ...
    #[cfg(windows)]
    {
        if bundled.exists() {
            if let Err(e) = std::fs::rename(&bundled, &dest) {
                log::warn!("Could not rename Parlo-cli.exe to Parlo.exe: {}", e);
            }
        }
    ...
    #[cfg(unix)]
    {
        let install_dir = parlo_cli_install_dir()?;
        std::fs::create_dir_all(&install_dir).map_err(|e| e.to_string())?;
        let dest = install_dir.join(dest_bin_name);

        std::fs::copy(&bundled, &dest)
            .map_err(|e| format!("Failed to copy Parlo to {}: {}", dest.display(), e))?;
...
/// Copy the bundled `Parlo` binary to the system PATH (Tauri command wrapper).
#[tauri::command]
pub async fn install_parlo_cli<R: Runtime>(
    app_handle: AppHandle<R>,
) -> Result<CliInstallStatus, String> {
    install_parlo_cli_sync(&app_handle)
}

/// Remove the installed `Parlo` CLI binary.
#[tauri::command]
pub fn uninstall_parlo_cli() -> Result<(), String> {
    #[cfg(windows)]
    {
        let bin_dir = parlo_cli_bin_dir_windows()?;
    ...
    #[cfg(unix)]
    {
        let dest = parlo_cli_install_dir()?.join("Parlo");
        if dest.exists() {
            std::fs::remove_file(&dest)
                .map_err(|e| format!("Failed to remove Parlo CLI from {}: {}", dest.display(), e))?;
        }
        Ok(())
    }
}

/// Build the cleaned shell-file content with all Parlo CC env vars stripped out.
fn build_cleaned_env_content(env_file_path: &str) -> String {
    let existing_content = std::fs::read_to_string(env_file_path).unwrap_or_default();
    let cleaned: Vec<&str> = existing_content
        .split('\n')
        .filter(|line| {
            !line.starts_with("# Parlo Local API Server - Claude Code Config")
                && !line.starts_with("# Parlo Local API Server")
                && !line.starts_with("export ANTHROPIC_")
        })
        .collect();
...
/// Clear all Parlo-written Claude Code environment variables from the shell config.
    ...
    if cfg!(target_os = "macos") {
            ...
            .open(&env_file_path)
        {
            ...
            Err(_) => {
                ...
                let temp_path = format!("{}/.parlo_env_clear.sh", home_dir);
...
/// Determine the best writable directory for the Parlo CLI install (Unix only).
#[cfg(unix)]
fn parlo_cli_install_dir() -> Result<PathBuf, String> {
    let usr_local_bin = PathBuf::from("/usr/local/bin");
    if usr_local_bin.exists() {
        let probe = usr_local_bin.join(".parlo_write_probe");
...
#[cfg(windows)]
fn parlo_cli_bin_dir_windows() -> Result<PathBuf, String> {
    let local_app_data =
...
fn add_to_path_windows(install_dir: &PathBuf) -> Result<(), String> {
    ...
    // Remove stale old-style PATH entry (..\\Programs\\Parlo without \\resources\\bin)
    // left by previous versions that placed Parlo.exe next to the GUI binary.
    let old_parlo_dir = install_dir
        .parent()
        .and_then(|p| p.parent())
    ...
    let parts: Vec<&str> = existing_user_path
        ...
        .filter(|p| !p.is_empty())
        .filter(|p| {
            if let Some(ref old) = old_parlo_dir {
...
mod tests {
    ...
    fn create_all_data(dir: &std::path::Path) {
        for subdir in PARLO_DATA_SUBDIRS {
            fs::create_dir_all(dir.join(subdir)).unwrap();
            fs::write(dir.join(subdir).join("dummy.txt"), "data").unwrap();
        }
        for file in PARLO_DATA_FILES {
    ...
    fn test_delete_conversations_only_removes_conversation_dirs() {
        ...
        assert!(!exists_any(d, PARLO_DATA_DIRS_CONVERSATIONS));
        assert!(exists_all(d, PARLO_DATA_DIRS_MODELS));
        assert!(exists_all(d, PARLO_DATA_DIRS_COMMON));
    ...
    fn test_delete_models_and_configs_only_removes_model_dirs_and_config_files() {
        ...
        assert!(!exists_any(d, PARLO_DATA_DIRS_MODELS));
        assert!(!exists_any(d, PARLO_DATA_FILES_CONFIGS));
        assert!(exists_all(d, PARLO_DATA_DIRS_CONVERSATIONS));
        assert!(exists_all(d, PARLO_DATA_DIRS_COMMON));
        assert!(d.join("settings.json").exists());
    }
    ...
    fn test_delete_common_data_only_removes_common_dirs() {
        ...
        assert!(!exists_any(d, PARLO_DATA_DIRS_COMMON));
        assert!(exists_all(d, PARLO_DATA_DIRS_CONVERSATIONS));
        assert!(exists_all(d, PARLO_DATA_DIRS_MODELS));
        assert!(d.join("settings.json").exists());
        assert!(d.join("mcp_config.json").exists());
    ...
    fn test_full_wipe_deletes_settings_json() {
        ...
        assert!(!d.join("settings.json").exists());
        assert!(!exists_any(d, PARLO_DATA_SUBDIRS));
        assert!(!exists_any(d, PARLO_DATA_FILES));
    }
    ...
    fn test_is_safe_to_delete() {
        ...
        assert!(is_safe_to_delete(std::path::Path::new("/home/user/Parlo")));
        assert!(is_safe_to_delete(std::path::Path::new(
            "/home/user/.local/share/Parlo"
        )));
    }
}
