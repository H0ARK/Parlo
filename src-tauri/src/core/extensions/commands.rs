...
#[tauri::command]
pub fn get_parlo_extensions_path<R: Runtime>(app_handle: tauri::AppHandle<R>) -> PathBuf {
    get_parlo_data_folder_path(app_handle).join("extensions")
}
...
pub fn get_active_extensions<R: Runtime>(app: AppHandle<R>) -> Vec<serde_json::Value> {
    ...
    {
        let mut path = get_parlo_extensions_path(app);
        path.push("extensions.json");
        log::info!("get Parlo extensions, path: {path:?}");
