...
async fn test_run_mcp_commands() {
    ...
    let app_path = get_parlo_data_folder_path(app.handle().clone());
...
fn test_extension_disconnected_error_detection() {
    // Real error messages from Parlo Browser MCP server when extension is not connected
...
fn test_mcp_lock_file_serde_round_trip() {
    ...
    let lock = McpLockFile {
        pid: 4242,
        port: 17389,
        server_name: "Parlo Browser MCP".to_string(),
    ...
    assert_eq!(parsed.server_name, "Parlo Browser MCP");
