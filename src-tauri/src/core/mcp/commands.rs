...
pub async fn deactivate_mcp_server<R: Runtime>(
    ...
    let bridge_port = if name == "Parlo Browser MCP" {
    ...
    // Delete lock file if this is Parlo Browser MCP and we have a port
...
pub async fn get_mcp_configs<R: Runtime>(app: AppHandle<R>) -> Result<String, String> {
    let mut path = get_parlo_data_folder_path(app.clone());
    ...
    // Migration: Add Parlo Browser MCP if not present
    let mcp_servers = config_object
        .get_mut("mcpServers")
        .and_then(|v| v.as_object_mut())
        .ok_or("mcpServers is not an object")?;

    if !mcp_servers.contains_key("Parlo Browser MCP") {
        log::info!("Migrating config: Adding 'Parlo Browser MCP' server");
        mcp_servers.insert(
...
/// Check if Parlo Browser extension is connected via MCP
#[tauri::command]
pub async fn check_parlo_browser_extension_connected(
    state: State<'_, AppState>,
) -> Result<bool, String> {
    let servers = state.mcp_servers.lock().await;
    let service = match servers.get("Parlo Browser MCP") {
        Some(s) => s,
