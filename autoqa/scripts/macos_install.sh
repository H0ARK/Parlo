#!/bin/bash
# macOS install script for Parlo app

echo "Installing Parlo app from DMG..."

# Mount the DMG
hdiutil attach "/tmp/Parlo-installer.dmg" -mountpoint "/tmp/Parlo-mount"

# Find the .app file in the mounted DMG
APP_FILE=$(find "/tmp/Parlo-mount" -name "*.app" -type d | head -1)

if [ -z "$APP_FILE" ]; then
    echo "[Failed] No .app file found in DMG"
    hdiutil detach "/tmp/Parlo-mount" || true
    exit 1
fi
...
# Unmount the DMG
hdiutil detach "/tmp/Parlo-mount"
...
POTENTIAL_EXECUTABLES=(
    ...
    "/Applications/$APP_NAME/Contents/MacOS/Parlo"
    "/Applications/$APP_NAME/Contents/MacOS/Parlo-nightly"
)
...
echo "PARLO_APP_PATH=$APP_PATH" >> $GITHUB_ENV
...
# Verify installation
if [ -f "$APP_PATH" ]; then
    echo "[SUCCESS] Parlo app installed successfully"
    ls -la "/Applications/$APP_NAME"
else
    echo "[FAILED] Parlo app installation failed - executable not found"
    exit 1
fi
