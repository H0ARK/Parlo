...
- [ ] Add 2 custom assistants to Parlo 
...
- [ ] [0.6.9] Take a `gguf` file and delete the `.gguf` extensions from the file name, import it into Parlo and verify that it works.
...
- [ ] [0.6.9] Make sure that Ollama set-up  as a custom provider work with Parlo
...
- [ ] [0.6.9] Make sure that you can send an image to a Local API Server and it also works (can set up Local API Server as a Custom Provider in Parlo to test)
- [ ] [0.6.10] Make sure you are still able to see API key when server local status is running
- [ ] [0.7.0] Users can see the Parlo API Server Swagger UI by opening the following path in their browser `http://<ip>:<port>`
...
- [ ] Clicking on a listing will open up the model card information within Parlo and render the HTML properly
...
## E. Assistants
- [ ] There is always at least one default Assistant which is Parlo
- [ ] The default Parlo assistant has `stream = True` by default 
...
- [ ] Click on `Reset` button in `Other` to factory reset the app:
	- [ ] All threads deleted
	- [ ] All Assistant deleted except for default Parlo Assistant
...
- [ ] Clean up by deleting all the left over folder created by Parlo
	- [ ] On MacOS
		- [ ] `~/Library/Application Support/Parlo`
		- [ ] `~/Library/Caches/Parlo.ai.app`
	- [ ] On Windows
		- [ ] `C:\Users<Username>\AppData\Roaming\Parlo\`
		- [ ] `C:\Users<Username>\AppData\Local\Parlo.ai.app`
	- [ ] On Linux
		- [ ] `~/.cache/Parlo`
		- [ ] `~/.cache/Parlo.ai.app`
		- [ ] `~/.local/share/Parlo`
		- [ ] `~/.local/share/Parlo.ai.app`
- [ ] Ensure that the fresh install of Parlo launch
