...
import { getParloDataFolderPath } from './core'
...
describe('test core apis', () => {
  ...
  it('should get Parlo data folder path', async () => {
    globalThis.core = {
      api: {
        getParloDataFolderPath: vi.fn().mockResolvedValue('/path/to/Parlo/data'),
      },
    }
    const result = await getParloDataFolderPath()
    expect(globalThis.core.api.getParloDataFolderPath).toHaveBeenCalled()
